...
function generateString(resource, upstreamRepresentation, res){
	var graph = resource.route.graph;
	const xml = new DOMParser({
		locator: { systemId: resource.uri },
		// errorHandler: function(level,msg){ throw new Error(msg); },
	}).parseFromString(upstreamRepresentation.body); // xsltString: string of xslt file contents
	assert(xml);
...
RouteApplyTheme.prototype.render_transform = async function render_transform(resource, req, input, output){

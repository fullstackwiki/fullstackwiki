...
//	}
	...
	// 	return map;
	// })();
	var upLink = self.graph.match(self.resource.euri, 'http://www.w3.org/1999/xhtml/vocab#up', null).toArray()[0];
	const tocSections = [];
	...
	const upHTML = (function(){
		...
		while(first){
			...
			console.log(self.resource.euri, first, label);
		...
		return '<ol>'+steps.map(function(li){
			return '<li><a href="'+escapeHTML(li.href)+'">'+escapeHTML(li.label)+'</a></li>'
		}).join('')+'</ol>';
	})();

...
function stripIndent(text){
	var leadingIndent = text.match(/^\n[\t ]*/)[0];
	var endCharacter = text.lastIndexOf("\n");
	return text.substring(leadingIndent.length, endCharacter).split(leadingIndent).join("\n");
}
...
Render.prototype._flush = function (callback){
	...
	function applyTemplate(doc){
		...
		if(doc.localName=='pre'){
			...
			const textContent = doc.getAttribute('w:space')=='indent' ? stripIndent(doc.textContent) : doc.textContent ;
			const hjsHTML = hjsLanguage ? hjs.highlight(hjsLanguage, textContent).value : textContent;

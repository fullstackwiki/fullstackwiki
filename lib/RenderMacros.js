...
function generateString(resource, upstreamResponse, res){
	assert(typeof upstreamResponse.body === 'string');
	...
	const document = new DOMParser().parseFromString(upstreamResponse.body); // xsltString: string of xslt file contents
...
RouteApplyMacros.prototype.render_transform = async function render_transform(resource, req, input, output){
	const message = await ResponseMessage.fromStream(input);
	assert(message.body);
	generateString(resource, message, output);
};
